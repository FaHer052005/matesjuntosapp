// Datos de ejemplo para la primera vez que abrís la app.
// Después todo vive en localStorage del navegador.

import { PMTS } from "../utils/constants";
  ...
  for (let d = 29; d >= 0; d--) {
    ...
    for (let j = 0; j < n; j++) {
      ...
      out.push({
        ...
        date: dt.toISOString(),
        items,
        paymentMethod: PMTS[Math.floor(Math.random() * PMTS.length)],
        total: tot,
        profit: prft,
        note: "",
      });
...
/** Lee localStorage o devuelve productos + ventas de ejemplo. */
export function loadAppData() {
  try {
    ...
    // Si el JSON está corrupto, seguimos con datos nuevos
  }

  const products = SEED_PRODUCTS;
  return {
    products,
    sales: genSales(products),
    fromSeed: true,
  };
}
